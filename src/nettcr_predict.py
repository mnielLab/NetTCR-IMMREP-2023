...
parser.add_argument("-m", "--model_path", help="Path to the directory containing models use for the predictions")
    ...
    for t in train_parts:
        ...
        for v in train_parts:
            if v!=t:      
                ...
                try:
                    interpreter = tf.lite.Interpreter(model_path = os.path.normpath(model_path+'/pre_trained/'+pep+'/checkpoint/'+'t.'+str(t)+'.v.'+str(v)+".tflite"))
                except ValueError as error:
                    #print(error)
                    print_flag += 1
                    # Load pan-specific TFLite model and allocate tensors.
                    interpreter = tf.lite.Interpreter(model_path = os.path.normpath(model_path+'/cdr123_pan/checkpoint/'+'t.'+str(t)+'.v.'+str(v)+".tflite"))
                    if print_flag == 1:
